...
- The `User-Agent` is a characteristic string that lets the servers and network peers identify the application operating system, vendor and/or version of the requesting user-agent.
...
HTTP request body is the data sent by the client. HTTP requests can optionally include a request body. A request body is often included when using the HTTP POST and PUT methods to transmit data.
